...
import { FiTrendingUp, FiTrendingDown, FiClock, FiDollarSign, FiZap, FiSmile, FiCheckCircle, FiActivity } from 'react-icons/fi';
...
function getResultIcon(title: string, icon?: string): React.ReactNode {
  ...
  if (lowerTitle.includes('tempo') || lowerTitle.includes('time') || lowerTitle.includes('dias') || lowerTitle.includes('day')) {
    return <FiClock className="h-6 w-6" />;
  }
  if (lowerTitle.includes('produtividade') || lowerTitle.includes('productivity') || lowerTitle.includes('rápido') || lowerTitle.includes('fast')) {
...
export function ResultsShowcase({ 
  ...
  if (variant === 'compact') {
    return (
      <div className="my-8">
        {title && (
          <h3 className="mb-4 text-xl font-bold text-base-content">{title}</h3>
        )}
